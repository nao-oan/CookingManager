...
| 2 | 主要4機能を下部タブに固定配置する | 在庫追加・食事記録を3タップ以内に収める（NFR-9） |
| 3 | 一覧 → 詳細 → 編集 の3階層までとし、それ以上深くしない | 迷子を防ぎ、戻り導線を単純に保つ |
| 4 | 新規作成・編集はモーダルではなく専用画面とする | 入力途中の離脱でデータを失わない。URLで状態を表現できる |
| 5 | 認証以外の全画面をログイン必須とする | F1-2 |
...
| タブ | 対応機能 | 理由 |
|---|---|---|
| ホーム | F6 レシピ提案 | **アプリを開く最大の動機**が「今日何を作るか」であるため、提案を起点に置く |
| レシピ | F3 レシピ登録 | — |
| 冷蔵庫 | F5 在庫管理 | 使用頻度が高い |
| 記録 | F4 食事記録 | — |
...
| ID | 画面名 | URL | 目的 | 機能要件 |
|---|---|---|---|---|
| H-1 | ホーム | `/` | 在庫から作れるレシピを不足の少ない順に見る | F6-1〜F6-7 |
...
| ID | 画面名 | URL | 目的 | 機能要件 |
|---|---|---|---|---|
| S-1 | 設定 | `/settings` | 各設定への入口。ログアウトする | F1-3 |
| S-2 | 食材一覧 | `/settings/ingredients` | 食材マスタを一覧・検索する | F2-1 |
| S-3 | 食材編集 | `/settings/ingredients/[id]/edit` | 食材を編集・削除する。常備食材を設定する | F2-1 |
...
| 機能要件 | 画面 |
|---|---|
| F1-1 登録・ログイン | A-1, A-2 |
| F1-2 自分のデータのみ | 全画面（ミドルウェアで担保） |
| F1-3 ログアウト | S-1 |
| F2-1 食材のCRUD | S-2, S-3 ／ 作成は R-3, R-4, P-2, P-3 の入力中 |
...
| F5-2 期限順の表示 | P-1 |
| F5-3 数量の直接増減 | P-1 |
| F5-4 期限3日以内の警告 | P-1（H-1 でも表示） |
| F5-5 期限切れの除外 | P-1, H-1 |
| F6-1 不足食材の算出 | H-1 |
| F6-2 不足の少ない順 | H-1 |
| F6-3 不足食材の表示 | H-1 |
| F6-4 常備食材の除外 | H-1（設定は S-3） |
| F6-5 同一食材の合計 | H-1 |
| F6-6 単位変換・数量不明 | H-1, R-2 |
| F6-7 空状態の案内 | H-1 |
...
| # | 導線 | 経路 |
|---|---|---|
| 1 | 今日の献立を決める | H-1 → R-2（詳細で材料を確認）→ 調理 |
| 2 | 作ったものを記録する | R-2 →「食べた記録をつける」→ M-3 |
| 3 | 買い物から帰って在庫を入れる | P-1 → P-2 |
| 4 | レシピを増やす | R-1 → R-3 |
| 5 | 常備食材を設定する | S-1 → S-2 → S-3 |

導線2が重要。**レシピ詳細から直接記録に飛べる**ことで、「作る → 記録する」が途切れない。記録タブを経由させると date と slot の選択が挟まり、NFR-9 の3タップを超える。
...
記録タブ経由は4タップになるが、レシピ詳細からの導線（導線2）が2タップで済むため、要件は主導線で満たす。当日の記録については、M-1 カレンダーで**今日を初期選択**しておき、1タップ削る。
...
|---|---|---|
| 公開 | A-1, A-2 | そのまま表示 |
| 要ログイン | 上記以外の全14画面 | `/login` へリダイレクトし、元のURLを復帰先として保持する |

- 判定は Next.js の middleware で行う（F1-2）
- ログイン済みユーザーが `/login`・`/signup` を開いた場合は `/` へリダイレクトする
...
| 宛先 | 内容 |
|---|---|
| 0-4 システム設計 | 画面ごとに必要なAPI・データ取得を定義する。特に H-1 は提案算出を伴うため、取得範囲とキャッシュ方針を決める |
| 0-5 画面設計 | 本書の16画面すべてについてワイヤーフレームを作成する。H-1・P-1・R-2 は表示ルールが複雑なため優先する |
...
| # | 論点 | 保留理由 |
|---|---|---|
| 1 | H-1 の提案を何件表示するか | 実データを見ないと適切な件数が判断できない。0-5 で暫定値を決め、フェーズ5で調整する |
