...
| A-3 | パスワード再設定の依頼 | `/reset-password` | 再設定用のリンクをメールで受け取る | F1-1 |
| A-4 | 新しいパスワードの設定 | `/reset-password/new` | メールのリンクから新しいパスワードを設定する | F1-1 |
...
**合計19画面**（公開4・要ログイン15）。A-4 はメールのリンクで張られた復旧セッションが前提のため、要ログイン側に数える。
